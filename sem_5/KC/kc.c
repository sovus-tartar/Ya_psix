...
int rabin_karp(const char *needle, const char *haystack)
{
  ...
  //printf("strlen = %lu right = %u\n", strlen(haystack), right);
  if (right > strlen(haystack))
  {
    return -1;
  }

  if (cur == target) {
    if (strncmp(haystack + left, needle, right - left) == 0)
        return left;
  }
  ...
  while (haystack[right] != 0)
  {
    ...
    if (cur == target)
    {
      //printf("left = %u\n needle = %s haystack = %.40s\n strncmp = %d\n", left, needle, haystack + left, strncmp(haystack + left, needle, length));
      if (strncmp(haystack + left, needle, length) == 0)
        return left;
    }
  }
